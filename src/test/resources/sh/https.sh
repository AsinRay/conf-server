...
curl --cacert cacert.pem  https:/root:toor@confserver.ailu.internal:8443/admin/ws/token
echo "\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"

# add new token of given app
curl --cacert cacert.pem  https:/root:toor@confserver.ailu.internal:8443/admin/add/ws/boot:boot@
echo "\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
curl --cacert cacert.pem https://boot:boot@confserver.ailu.internal:8443/ws/dev
